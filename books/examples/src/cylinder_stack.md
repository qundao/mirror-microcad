...
part CylinderStack(heights: [Length], radii: [Length], offset: Length = 0mm) {
    assert(
        heights.count() == radii.count() - 1,
        "radii must be an array with one more elements that heights",
    );

    self = Cylinder(
        height = heights.head(),
        radius_bottom = radii.head(),
        radius_top = radii.tail().head(),
        offset,
    );
    if heights.count() > 1 {
        self | CylinderStack(
