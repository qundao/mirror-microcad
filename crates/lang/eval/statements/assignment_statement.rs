...
impl Eval for AssignmentStatement<ValueAssignment> {
    fn eval(&self, context: &mut EvalContext) -> EvalResult<Value> {
        log::debug!("Evaluating assignment statement:\n{self}");

        let new_value: Value = self.assignment.eval(context)?;

        // apply any attributes to any model value
        let new_value = match new_value {
            Value::Model(model) => {
                model.set_id(self.assignment.id());
        ...
        if let Ok(symbol) = context.lookup(
            &QualifiedName::from_id(self.assignment.id()),
            LookupTarget::Value,
        ) {
            let err = symbol.with_def_mut(|def| match def {
                        ...
                        *value = new_value.clone();
                        ...
                        Some((
                            ...
                            EvalError::ValueAlreadyDefined {
                                location: self.assignment.src_ref(),
                                name: id.clone(),
                ...
                _ => Some((
                    self.assignment.id(),
                    EvalError::NotAnLValue(self.assignment.id()),
                )),
            });
...
impl Eval<Value> for Assignment {
    fn eval(&self, context: &mut EvalContext) -> EvalResult<Value> {
        // evaluate assignment expression
        let new_value: Value = self.expression.eval(context)?;
        if let Err(err) = self.type_check(new_value.ty()) {
            context.error(self, err)?;
            Ok(Value::None)
        } else {
            Ok(new_value)
